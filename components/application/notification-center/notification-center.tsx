...
export type NotificationCenterTab = "all" | "bounties" | "referrals";
...
export function NotificationCenter({
  ...
  const isUnread = (item: NotificationCenterItem) => item.unread === true && (onMarkRead ? true : !readIds.has(item.id));
  const unreadCount = notifications.filter(isUnread).length;

  const tabCounts = useMemo(
    () => ({
      all: notifications.length,
      bounties: notifications.filter((item) => item.category === "bounties").length,
      referrals: notifications.filter((item) => item.category === "referrals").length,
    }),
    [notifications],
  );
                            ...
                            {item.actions.map((action) => (
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  else setReadIds((current) => new Set(current).add(item.id));
