...
export function ThemeToggle({
        ...
        className={cx(
          ...
          glass
            ? // No track of its own: the host supplies the surface. A fill
              // here would paint over it. Kept registry-safe â€” this component
                ...
                glass
                  ? selected
                    ? // Reads against the selected pill, which is white in
                      // light and dark grey in dark â€” so it flips with it.
  ...
  // A menu item should remain a straightforward button. React Aria's Switch
  // renders a nested hidden input, which can be perceived as disabled inside
  // an account menu in some browsers. This keeps the BoardUI track while
  // giving the whole row a reliable pointer and keyboard activation target.
  return (
    <button
      type="button"
      ...
      className={cx(
        ...
        "text-left transition-colors duration-150 ease hover:bg-background-secondary-hover",
    ...
    >
      ...
      <SwitchTrack
        state={{ isSelected: dark, isDisabled: false, isFocusVisible: false }}
        size="sm"
        shape="pill"
      />
    </button>
