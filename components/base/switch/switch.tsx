...
/**
 * Figma source: Board UI â†’ "Toogles" (node 3856:3807).
 *
 * Two shapes Ã— three sizes, plus off / on:
 *   shape   pill        (Figma "fully rounded") â€” track & thumb rounded-full
 *           rectangle   (Figma "rectangle")     â€” track radius/md-scale, thumb radius/xs-scale
 *   size    sm 28Ã—16 Â· md 42Ã—24 Â· lg 56Ã—32   (track wÃ—h; thumb = h âˆ’ 2Â·pad)
 *
 * Anatomy (all values 1:1 with Figma):
 *   track   off â†’ bg background/tertiary/default (#ebebeb, no border)
 *           on  â†’ Button/Primary gradient (blue/500 â†’ blue/600) + an inset
 ...
 *           slides by (w âˆ’ thumb âˆ’ 2Â·pad) between states.
 *   chip    small embossed inset in the thumb centre:
 *           off â†’ whiteâ†’neutral-100 in light mode; in dark mode it matches
 *                 the track so the mark reads like a hole through the thumb;
 *           on  â†’ switch-on-chip-startâ†’end (accent mixes matching Figma's
 *                 raw #2473feâ†’#0450e2 for blue), accent/600 border.
 *
...
/**
 * The track + thumb + chip visual. Pure presentation â€” the react-aria Switch
...
export function SwitchTrack({
  ...
  return (
    ...
    >
      ...
      >
        {/* Embossed inner chip. Gradient runs bottomâ†’top (`to-t`) â€” equivalent
            to the Figma layer's vertical flip â€” but WITHOUT a transform, so the
