...
/**
 * Figma source: Board UI â†’ Buttons (node 3656:13819).
 *
 ...
 *   gap                 2px                       2px                      1.33pxâ†’1
 *   icon                20Ã—20                     18Ã—18                    14Ã—14
 *   label wrapper       px=4                      px=2                     px=2
 *   text style          Body 1/Medium             Body 1/Medium            Caption 1/Semibold
 *   icon-only square    36Ã—36 (content-derived)   32Ã—32 (forced size)      24Ã—24 (forced size)
 *
 * `xs` is the smallest tier â€” first needed for the calendar template's
...
const styles = sortCx({
  ...
  // Icon-only override:
  //   Medium â†’ keep p-2; w expands from content (8+20+8 = 36) â†’ square.
  //   Small  â†’ Figma forces 32Ã—32 even though 8+18+8=34, so we hard-set size-8
  //            and zero the padding; the inner flex centers the 18px icon.
  //   Xs     â†’ forces 24Ã—24, content-centered â€” used for the calendar
  //            template's edit-icon buttons (timezone/participants/reminder).
  iconOnlySize: {
    medium: "",                // base size already produces 36Ã—36 with a 20px icon
    small:  "size-8 p-0",      // hard 32Ã—32, content-centered
    xs:     "size-6 p-0",      // hard 24Ã—24, content-centered
  },
