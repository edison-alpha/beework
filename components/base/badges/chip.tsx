...
import { SkillIcon, hasSkillIcon } from "@/components/foundations/icons/skill-icons";
import { cx, sortCx } from "@/utils/cx";

/**
 * Figma source: Board UI â†’ dashboard 1 "Chip" layers (status chips, delta
 * chips, price chips, role chips â€” e.g. nodes 3731:3273, 3731:3094, 3731:3280,
 * 3731:3055).
 *
 * Colored label chip. Three emphasis levels, all rounded radius/md (6px),
 * px 6:
 *
 *   bold    py 2, Body 1/Medium   (14/20/500) â€” status + percentage deltas
 *   subtle  py 4, Body 1/Medium   (14/20/500) â€” price chips
 *   caption py 4, Caption 1/Medium (12/16/500, tracking .15) â€” role tags
 *
 * Status pairs are semantic so the same component follows both Figma modes:
 *   lime    light 200/800 Â· dark 950 @ 60% / 500
 *   rose    light 200/800 Â· dark 950 @ 60% / 500
 *   yellow  light 200/800 Â· dark 950 @ 60% / 500
 *   cyan    light 200/800 Â· dark 950 @ 60% / 400
 *   blue    light 200/800 Â· dark 950 @ 60% / 300 (same dark status recipe;
 *           first needed by the medical template's "In treatment" status)
 *   purple  light 100/600 Â· dark 900/300 (AI profile delta chips)
...
const styles = sortCx({
  base: "inline-flex items-center justify-center gap-1 rounded-md px-1.5 whitespace-nowrap transition-[padding,font-size] duration-200 ease",
  variant: {
...
export function Chip({
  ...
  return (
    <span
      ...
      {...props}
    >
      {label && hasSkillIcon(label) ? <SkillIcon skill={label} /> : null}
      {children}
    </span>
  );
}
