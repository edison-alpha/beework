...
/**
 * Figma source: Board UI â†’ Input (node 3665:1849).
 *
 * Architecture (mirrors the Untitled UI / React Aria pattern):
 *
 *   InputBase   â€” the styled field shell. Renders <AriaGroup> + adornments
 *                 + <AriaInput>. State (hover / focus / disabled / invalid)
 *                 arrives as render-prop booleans from Aria.
 *   TextField   â€” wraps <AriaTextField> and a TextFieldContext so size /
 *                 classes can flow from the composer down into InputBase
 *                 without prop drilling.
 *   Input       â€” opinionated composition: TextField â†’ Label â†’ InputBase
 *                 â†’ HintText. The component most consumers use.
 *
 * Why React Aria:
 *   - Label â†” input â†” hint association via aria-labelledby / aria-describedby
 ...
 *     `isInvalid`, `isRequired` â€” no CSS pseudo gymnastics
 *
 * Visuals stay 1:1 with Figma â€” see `styles/theme.css` for the tokens.
 */
...
const inputStyles = sortCx({
  ...
  fieldSize: {
    medium: "p-2",                // 8px all sides â†’ h auto = 36
