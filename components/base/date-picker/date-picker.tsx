...
/**
 * Figma source: Board UI â†’ "Calendar_single" (node 3879:6708).
 *
 * A single-date picker: trigger button opens a popover with one month
 * (react-aria's plain `Calendar`, so both nav chevrons are fully functional â€”
 ...
 * date chip, and trigger/popover surfaces built for `DateRangePicker` â€” see
...
export function DatePicker({
  ...
  // keying `Calendar` on it forces a fresh mount showing the right month â€”
  ...
  // committed value every time the popover opens â€” not just when DatePicker's
    ...
    <>
      {!isExternal && (
        <button
          ...
          className={cx("date-picker-scale", triggerButtonClassName, className)}
        >
      ...
      <Popover
        ...
        isNonModal
        className={cx("date-picker-scale", popoverClassName)}
      >
