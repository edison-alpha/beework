...
/**
 * Shared building blocks for the base/date-picker family â€” `DateRangePicker`
...
/** Exact paths from Figma's month-nav chevrons (node 3869:5461 / 3869:5528) â€” a
 *  16Ã—16 glyph, 2px round-capped stroke, mirrored around x=8. */
...
export function DayCell(props: CalendarCellRenderProps & { isRange: boolean }) {
  ...
  // A plain (non-range) Calendar never sets isSelectionStart/isSelectionEnd â€”
  ...
  // extending half of it (6px) toward each selected neighbor â€” never past
  ...
  // toggling opacity is a plain CSS transition, not a mount/unmount â€” that
...
export function MonthPanel({
   ...
   *  be embedded directly inside a caller-styled container instead â€” used by
   *  the calendar template's inline month switcher, which supplies its own
   *  card (matching a different surface's border/shadow). */
  bare?: boolean;
  /** Skip the title + prev/next row entirely â€” used when a caller already
  ...
  // (DatePicker) â€” exactly one of these contexts is non-null depending on
...
/** Shared trigger-button chrome for both pickers (Figma's "Primary" button â€”
