...
export function BountyCard({ bounty, activity }: { bounty: Bounty; activity?: { kind: "published" | "submitted" | "won"; date: string } }) {
  const activityLabel = activity?.kind === "published" ? "Published" : activity?.kind === "submitted" ? "Submitted" : "Won";
  return (
    <motion.article layout initial={{ opacity: 0, y: 5 }} animate={{ opacity: bounty.status === "completed" ? .55 : 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: .18 }} className="group rounded-xl border border-border-button-default bg-background-primary-default transition-colors hover:bg-background-secondary-default">
      <Link href={`/bounties/${bounty.slug}`} className="focus-ring grid min-h-[92px] grid-cols-[48px_minmax(0,1fr)] items-center gap-3 rounded-xl p-2.5 sm:grid-cols-[52px_minmax(0,1fr)_145px] sm:gap-4">
        ...
        <div className="min-w-0 self-center">
          ...
          <div className="mt-2 flex gap-1.5 overflow-hidden">{activity && <Chip variant="caption" color={activity.kind === "won" ? "yellow" : "soft"}>{activity.kind === "won" && <Icon icon="solar:cup-star-bold" className="mr-1 size-3.5" aria-hidden="true" />}{activityLabel}</Chip>}{bounty.eligibility === "Verified talent" && <Chip variant="caption" color="soft">Verified only</Chip>}<Chip variant="caption" color="soft">{bounty.category}</Chip>{bounty.skills.slice(0, 1).map((skill) => <Chip key={skill} variant="caption" color="soft" className="hidden sm:inline-flex">{skill}</Chip>)}</div>
        </div>
